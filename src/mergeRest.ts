...
export const mergeRest = (grunaviRest: GrunaviRest[], hotpepperRest: HotpepperRest[]): GrunaviRest[] => {
  ...
  for (const hotpepperEntry of hotpepperRest) {
    ...
    grunaviRest.forEach((item, index) => {
      ...
      if (index === (grunaviRest.length - 1) && !include && !addNameList.includes(hotpepperEntry.name)) {
        include = false;
        grunaviRest.push(
          {
